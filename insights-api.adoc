...
* <<ai-mcp-integration,AI / MCP Integration (optional)>>
...
NOTE: Filtering by staleness status ensures that only relevant systems are queried (e.g., `&staleness=fresh` or `&staleness=stale`).
...
[[cost-management]]
=== Cost management
Red Hat Lightspeed Cost Management provides visibility into cloud and infrastructure spend.

- https://github.com/project-koku/costmgmt-api-cheatsheet/blob/master/costmgmt-api-cheatsheet.pdf[API cheat sheet] - For API-based access to cost data (recommended for automation and reporting).

- https://developers.redhat.com/cheat-sheets/red-hat-lightspeed-cost-management-cheat-sheet[Getting started guide] - For initial setup and usage of the Cost Management service.
...
The Model Context Protocol (MCP) provides a standard way to connect APIs to LLM-based tools and agents.

An example implementation for Red Hat Lightspeed is available on https://github.com/RedHatInsights/insights-mcp[insights-mcp] GitHub repository.
